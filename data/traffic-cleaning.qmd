...
# Created using assistance from ChatGPT
# .groups="drop" method
mydata %>% 
  mutate(Month = month(mdy_hms(crash_date), label=TRUE)) %>% 
...
This variable counts the total injuries of the recorded collision(s). The values range from 0 to 21 total injuries, with a mean of 0.38. We added a secondary categorical variable, `injuries_total_cat`, to consolidate the entries of ten injuries in a single collision or more, since each entry from 10-21 has single digit occurances.
...
mydata <- mydata %>% mutate(
  injuries_total_cat = case_when(
    injuries_total < 10 ~ as.character(injuries_total),
    injuries_total >= 10 ~ "10+"
  ),
  injuries_total_cat = factor(injuries_total_cat, levels=c(as.character(1:9), "10+")))

table(mydata$injuries_total_cat)
